...
const NewPublication: FC<Props> = ({ publication }) => {
  ...
  const [broadcast] = useBroadcastMutation({
    onCompleted: (data) => {
      ...
      if (data.broadcast.__typename === "RelayerResult") {
        setTxnQueue([
          generateOptimisticPublication({ txId: data.broadcast.txId }),
          ...txnQueue,
        ]);
      }
    },
      ...
      if (data.createCommentViaDispatcher.__typename === "RelayerResult") {
        setTxnQueue([
          generateOptimisticPublication({
            txId: data.createCommentViaDispatcher.txId,
          }),
          ...txnQueue,
        ]);
      }
  ...
  const createPublication = async () => {
    ...
    try {
      ...
      const attachmentsInput: LensterAttachment[] = attachments.map(
        (attachment) => ({
  ...
  return (
    <Card className={clsx({ "border-none rounded-none": !isComment }, "pb-3")}>
      ...
      {publicationContentError && (
        <div className="px-5 pb-3 mt-1 text-sm font-bold text-red-500">
          {publicationContentError}
        </div>
      )}
